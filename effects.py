...
class RainbowTest(Effect):
	...
	rainbow_colors = [color_violet, color_indigo, color_orange, color_red, color_green, color_blue, color_yellow]
	...
	def run(self):
		...
		while time.time() - start_time < 5:
			for y in range(self.wall.height):
				color = RainbowTest.rainbow_colors[(y + cycle) % len(RainbowTest.rainbow_colors)]
				for x in range(self.wall.width):
					r = (color & 0xff0000) >> 16
					g = (color & 0x00ff00) >> 8
					b = color & 0x0000ff
					h, s, v = colorsys.rgb_to_hsv(r / 255.0, g / 255.0, b / 255.0)
					self.wall.set_pixel(x, y, (h,s,v))
				self.wall.draw()
			cycle += 1
		time.sleep(2)
